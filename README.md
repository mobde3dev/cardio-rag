...
CardioRAG is a clinical question-answering application with a Next.js web interface, a FastAPI retrieval service backed by Supabase, and a deterministic clinical guideline PDF parsing & chunking ingestion pipeline.

## Project structure

`	ext
cardio-rag-backend/     FastAPI retrieval API
cardio-rag-pipeline/    Medical Guideline PDF Parsing & Chunking Pipeline
...
### Pipeline: PDF Parsing & Ingestion (Optional / Data Prep)

PowerShell:

`powershell
Set-Location cardio-rag-pipeline
python -m venv .venv
.\.venv\Scripts\Activate.ps1
